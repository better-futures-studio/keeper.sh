...
| ALLOWED_SIGNUP_DOMAINS         | `api`         | Optional. Comma-separated email domains that may register. Compared case-insensitively after trimming whitespace. When set, every sign-up path (Google, Microsoft, username/email) is rejected unless the address is on this list. Existing users can still sign in. An empty value leaves registration open.<br><br>e.g. `heyjet.ai,example.com` |
| SOCIAL_LOGIN_PROVIDERS         | `api`         | Optional. Comma-separated subset of `google,microsoft` to offer as sign-in methods. When unset, every provider that has credentials is offered. When set, only the listed providers are registered for login, even if the other provider's credentials are present. Unknown values fail at boot. Calendar source connections keep using `GOOGLE_*` / `MICROSOFT_*` directly and are not affected.<br><br>e.g. `google` |
| CREDENTIAL_LOGIN_ENABLED       | `api`         | Optional. Boolean that turns username and password registration and sign-in on or off. Defaults to `true`. Set to `false` when the instance should offer only social login (for example a Google Workspace deployment). The API then omits the username/password plugin, reports `credentialMode` as `none`, and rejects credential routes. Boot fails if no social provider remains enabled. |
